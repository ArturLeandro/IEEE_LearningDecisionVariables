...
def saveXLSX(algorithm, result, problem_label, scale, num_objectives):    
    ...
    if not os.path.exists('Results'):
        ...
        os.makedirs('./Results')
    var2.to_excel('./Results/optim_exp_'+str(algorithm)+'_'+problem_label+'_'+str(num_objectives)+'_'+str(scale)+'.xlsx',header=False, index=False)
...
def generateDVLSpreadSheets(problem_labels, model_labels, scales, samples, iterations, num_objectives, times):
...
def generateNSGASpreadSheets(problem_labels, evaluations, num_objectives, times):
